...
def generate_report_enhanced(transcription, filler_words, speech_rate, sentiment, body_language, audio_quality):
    ...
    if filler_words.get('score', 3) <= 3:
        top_fillers = Counter([occ['word'].lower() for occ in filler_words.get('occurrences', [])])
        common_fillers = ', '.join([f"'{w}'" for w, _ in top_fillers.most_common(3)])
        if common_fillers:
            general_recommendations.append(f"Use fewer filler words like {common_fillers}. Try pausing briefly instead of filling gaps with sounds.")

    if speech_rate.get('score', 3) <= 3:
        if speech_rate.get('average_rate', 150) > 200:
            general_recommendations.append("You're speaking quite fast. Try slowing down slightly to maintain clarity.")
        elif speech_rate.get('average_rate', 150) < 100:
            general_recommendations.append("Your speech is slower than average. Try picking up the pace a little for better engagement.")
        if len(speech_rate.get('pauses', [])) > 5:
            general_recommendations.append("There are frequent long pauses. Consider smoother transitions to keep the flow.")

    if sentiment.get('score', 3) <= 3:
        if sentiment.get('sadness_ratio', 0) > 20:
            general_recommendations.append("Your tone reflects noticeable sadness. Try balancing it with confident or optimistic expressions.")
        elif sentiment.get('negative_ratio', 0) > 10:
            general_recommendations.append("Some expressions came off as negative. Aim for a more constructive or uplifting tone when possible.")
